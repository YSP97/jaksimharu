...
## 기획 배경
스터디원 모집이 오픈카톡, 에브리타임 등 여러 채널에 분산되어 있어 위치 기반으로 빠르게 찾기 어렵고, 모집 후 소통 채널도 외부로 이탈하는 문제가 있었습니다. 스터디 장소 탐색도 별도로 이루어져 불편함이 컸습니다. 이를 하나의 서비스로 해결하고자 기획했습니다.

## 주요 기획 결정 사항
### 홈 화면 구성
스터디 모집 + 질문 게시글을 함께 노출하는 안도 검토했으나, 두 게시글의 위치 필터링 로직이 달라 개발 복잡도가 증가하고 위치 기반 매칭이라는 서비스 정체성이 희석될 우려가 있어 스터디 모집 게시글만 노출하는 방향으로 결정했습니다.
### 채팅방 생성 방식
당근마켓·번개장터 레퍼런스를 참고해 채팅방을 별도 생성하는 대신 게시글 작성 완료 시 자동 생성되도록 기획했습니다. 모집 인원 마감 시 참가 버튼이 자동 비활성화되어 관리 편의성과 개발 효율을 동시에 확보했습니다.
### 주소 변경 위치
배달의민족·당근마켓 등 위치 기반 서비스 레퍼런스를 제시해 마이페이지가 아닌 홈 화면 상단에서 즉시 변경할 수 있도록 기획했습니다. 타 지역 스터디원을 탐색하는 니즈를 고려한 결정입니다.
...
<!-- gif 추가 부탁드려요! -->
<img src= 'https://github.com/user-attachments/assets/c44fae5e-5e72-41b0-8874-bc43c9dca2ac' width='300px'/>
<img src= 'https://github.com/user-attachments/assets/75ed44db-49dd-4c27-9de6-d35422bb523e' width='300px'/>
...
| 회원가입 인증                | 스터디 게시글 작성, 상세, 수정 | 이메일 확인 페이지 | -                 |
